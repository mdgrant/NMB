...
<font size = 4> Table `r table_n`. Characteristics of studies enrolling adults. </font>

```{r studyCharRev}
## studyCharRev --------------------------------------- (2022-03-07 20:32) @----
study_char_diag.dat <- study_char.dat %>% 
  filter(refid %in% diag_refids) %>% 
  relocate(design, .before = study) %>% 
  arrange(design)

study_char_table_diag <- study_char_diag.dat %>%
  filter(
    # design %in% c("rct", "crossover"),
    age == "Adult",
    refid %in% diag_refids,
  ) %>%
  mutate(
...
countries <- study_char_table_diag %>% 
  tabyl(country) %>% 
  arrange(desc(n)) %>% 
  mutate(
...
## table_diag ------------------------------------------ (2022-03-09 07:23) @----
table_diag <- study_char_table_diag %>% 
...
# write_csv(table_diag, "/Users/mgrant/Desktop/table_ptout.csv", na = "")
...
# write_csv(study_char_table_diag, "/Users/mgrant/Desktop/study_char_table_diag.csv", na = "")
table_n <- tab_inc()
```
...
<font size = 4> Table `r table_n`. Patient characteristics of studies enrolling adults. </font>

```{r ptCharRev}
## (updated 2021/07/21 08:26) surgery type --------------------------------
surg <- study_char_diag.dat %>%
  filter(
    # design %in% c("rct", "crossover"),
    age == "Adult",
    refid %in% diag_refids,
...
pt_char_table_diag <- study_arm.dat %>%
  filter(
    # design %in% c("rct", "crossover"),
    age == "Adult",
    refid %in% diag_refids
...
# write_csv(pt_char_table_diag, "/Users/mgrant/Desktop/pt_char_table_diag.csv", na = "")
table_n <- tab_inc()
```
