...
study_arm_conf.dat <- study_arm.dat %>%
  ...
  select(refid, extbconf, tofrext, arm_id, year, design, arm_n, arm, age, depth_rev_f, nmbagent, volatile, mon_type, mon_cat, mon_cat_f, sug_dose)

# describe
# tab_lst(study_arm_conf.dat, c(tofrext, sug_dose, volatile, depth_rev_f:mon_cat_f))
...
conf_meta_temp <- dichot_conf.dat %>%
  mutate(
    ...
    confirm = case_when(
      extbconf == "yes" & tofrext %in%  c("tofrext9", "tofrext1") ~ "≥0.9 confirmed",
...
~mon_cat_f,~refid,~study,~year,~age,~arm,~arm_n,~rnmb9_n,~rnmb_where,~rnmb_location,~extbconf,~tofrext,~confirm,~nmbagent,~volatile,~depth_rev_f,~depth,~study_l,~design_f,
"Quant",3270,"Domenech 2019",2019,"Adult","Sug",42,0,"pacu","pacu","ns",NA,"Not stated","various",NA,"ns","NS",NA,"Retro Cohort",
"Quant",3270,"Domenech 2019",2019,"Adult","Neo",9, 0,"pacu","pacu","ns",NA,"Not stated","various",NA,"ns","NS",NA,"Retro Cohort"
)

## conf_meta ------------------------------------------ (2022-04-01 08:37) @----
...
rm(add, remove, temp_1, temp_2, conf_meta_temp)
```
...
<font size = 4> Table `r table_n`. Study arms included in analyses of TOFR ≥0.9 confirmed prior to extubation (sugammadex used for reversal). All studies utilized quantitative monitoring.</font>
...
conf_kbl %>% 
  ...
  mutate(
  rnmb_bar = case_when(
    confirm == "≥0.9 confirmed" ~ bar_wn(rnmb9_per, rnmb9_per, color_1, 1),
...
kbl(
  ...
  pack_topnoind(., "≥0.9 confirmed", 1, 8) %>%
  pack_topnoind(., "Not confirmed", 9, 20) %>%
  pack_topnoind(., "Not stated", 21, 6) %>%
  footnote(
    general = "RCT: randomized controlled trial; NR Trial: nonrandomized trial; Prosp Cohort: prospective cohort; Retro Cohort: retrospective cohort; ROC: rocuronium; Vec: vecuronium; Var: various; Min: minimal; Mod: moderate; NS: not stated; PACU: post-anesthesia care unit.",
...
```{r metaSug, fig.align = "left", fig.width = 9.5, fig.height = 10}
...
| Comparison                                  | RR (95% CI)       |
|:--------------------------------------------|:------------------|
| Confirmed vs. not confirmed                 | 0.08 (0.00, 0.64) |
| Removing Alday 2019                         | 0.12 (0.00, 0.92) |
| Assuming all not stated were confirmed      | 0.09 (0.00, 0.65) |
| Assuming all not stated were not confirmed  | 0.15 (0.00, 1.21) |
...
colorlist1 <- list("No RNMB" = "#a4d9a7", "RNMB" = "#ee4c6c")
colorlist2 <- list("No RNMB" = "#a4d9a7", "RNMB" = "#ee4c6c")
# colorlist2 <- list("No RNMB" = "#a4d9a7", "Not confirmed" = "#ee4c6c", "Confirmed" = "#f5b7b1")
data1 <- list("No RNMB" = 0.99, "RNMB" = .01)
data2 <- list("No RNMB" = 0.96, "RNMB" = .04)

svg("docs/assets/sugnoconfirm.svg",  width = 5, height = 5)
personograph(data1, color = colorlist1, n.icons = 100, fig.title = "TOFR ≥0.9 Confirmed")
dev.off()

svg("docs/assets/sugconfirm.svg", width = 5, height = 5)
personograph(data2, color = colorlist2, n.icons = 100, fig.title = "TOFR ≥0.9 Not Confirmed")
dev.off()
```


```{r personographSug, figures-side, fig.show="hold", out.width="100%", fig.width=10}
library(cowplot)
ggdraw() + 
  draw_image("docs/assets/sugnoconfirm.svg", width = 0.5) + 
  draw_image("docs/assets/sugconfirm.svg", width = 0.5, x = 0.5)
```
...
<font size = 4> Table `r table_n`. Study arms included in analyses of TOFR ≥0.9 confirmed prior to extubation (neostigmine used for reversal). All studies utilized quantitative monitoring.</font>
...
kbl(
  ...
  pack_topnoind(., "≥0.9 confirmed", 1, 8) %>%
  pack_topnoind(., "Not confirmed", 9, 20) %>%
  pack_topnoind(., "Not stated", 21, 29) %>%
  footnote(
    general = "RCT: randomized controlled trial; NR Trial: nonrandomized trial; Prosp Cohort: prospective cohort; Retro Cohort: retrospective cohort; ROC: rocuronium; Vec: vecuronium; Var: various; Min: minimal; Mod: moderate; NS: not stated; PACU: post-anesthesia care unit.",
    alphabet = c("Extubation includes designated in OR."),
    general_title = "",
...
```{r metaNeo, fig.align = "left", fig.width = 9.5, fig.height = 10.2}
...
| Comparison                                  | RR (95% CI)       |
|:--------------------------------------------|:------------------|
| Confirmed vs. not confirmed                 | 0.15 (0.06, 0.35) |
| Assuming all not stated were confirmed      | 0.26 (0.09, 0.62) |
| Assuming all not stated were not confirmed  | 0.22 (0.08, 0.51) |
...
colorlist2 <- list("No RNMB" = "#a4d9a7", "RNMB" = "#ee4c6c")
# colorlist2 <- list("No RNMB" = "#a4d9a7", "Not confirmed" = "#ee4c6c", "Confirmed" = "#f5b7b1")
data1 <- list("No RNMB" = 0.94, "RNMB" = .06)
data2 <- list("No RNMB" = 0.67, "RNMB" = .33)

svg("docs/assets/neonoconfirm.svg",  width = 5, height = 5)
personograph(data1, color = colorlist1, n.icons = 100, fig.title = "TOFR ≥0.9 Confirmed")
dev.off()

svg("docs/assets/neoconfirm.svg", width = 5, height = 5)
personograph(data2, color = colorlist2, n.icons = 100, fig.title = "TOFR ≥0.9 Not Confirmed")
dev.off()
```


```{r personographNeo, figures-side, fig.show="hold", out.width="100%", fig.width=10}
library(cowplot)
ggdraw() + 
  draw_image("docs/assets/neonoconfirm.svg", width = 0.5) + 
  draw_image("docs/assets/neoconfirm.svg", width = 0.5, x = 0.5)
```
