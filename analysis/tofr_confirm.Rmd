...
<font size = 4> Figure `r figure_n`. Residual neuromuscular blockade (TOFR < 0.9, pooled incidence proportion) in arms of randomized and nonrandomized studies according to type of monitoring and confirmation of TOFR prior to extubation.</font>
...
forester(left_side_data = table[,1:4],
         ...
         estimate_col_name = "RNMB per 100",
         ci_low = table$`CI low`,
...
<img src = "assets/confirm_summary.png" style="width:600px;" align = "left"/>

RNMB: residual neuromuscular blockade (TOFR < 0.9)
...
kbl(
  ...
  pack_top(., "No quantitative monitoring", 31, 35) %>%
  pack_sub_bold_ind(., "Qualitative monitoring", 31, 31) %>%
  pack_sub_bold_ind(., "Clinical/None/NS", 32, 35) %>%
  footnote(
...
# only quantitative monitoring
conf_meta_quant <- conf_meta_collapsed |>
...
```{r metaSug, fig.align = "left", fig.width = 9.5, fig.height = 12}
...
kbl(
  ...
  pack_top(., "No quantitative monitoring", 42, 60) %>%
  pack_sub_bold_ind(., "Qualitative monitoring", 42, 50) %>%
  pack_sub_bold_ind(., "Clinical/None/NS", 51, 60) %>%
  footnote(
    general = "RCT: randomized controlled trial; NR Trial: nonrandomized trial; Prosp Cohort: prospective cohort; Retro Cohort: retrospective cohort; ROC: rocuronium; Vec: vecuronium; Var: various; Min: minimal; Mod: moderate; NS: not stated; PACU: post-anesthesia care unit; NS: not stated.",
    alphabet = c("Extubation includes designated in OR."),
    general_title = "",
...
```{r metaNeo, fig.align = "left", fig.width = 9.5, fig.height = 17}
