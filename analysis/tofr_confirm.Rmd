...
conf_meta_temp <- dichot_conf.dat |> 
  ...
  mutate(
    ...
    confirm = ifelse(mon_cat_f == "Qual", "Qualitative monitoring", confirm), 
...
conf_meta <- bind_rows(conf_meta_temp, domenech2019) %>% 
  mutate(
    confirm = factor(confirm, levels = c("Confirmed ≥0.9", "Confirmed ≥0.8/NS", "Not confirmed", "Not stated", "Qualitative monitoring", "Clinical/None/NS")),
    rnmb_where = case_when(
    rnmb_where == "ns" ~ "NS",
...
conf_kbl %>%
  ...
  rnmb_bar = case_when(
    ...
    confirm == "Qualitative monitoring"  ~ bar_wn(rnmb9_per, rnmb9_per, "#e74343", 1),
...
kbl(
  ...
  pack_top(., "Qualitative monitoring", 31, 31) %>%
...
conf_meta_collapsed <- conf_meta |> 
  # filter(!confirm %in% c("Qualitative monitoring", "Clinical/None/NS")) |> 
  mutate(
...
rnmb_sug <- update(rnmb_meta_glmm_9,
  ...
  # subset = arm == "Sug" & confirm == "Qualitative monitoring"
  ...
  # subset = arm == "Neo" & confirm == "Qualitative monitoring"
...
kbl(
  ...
  pack_top(., "Qualitative monitoring", 42, 50) %>%
