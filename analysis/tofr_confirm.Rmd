...
# collapse multiarm
remove <- c(1511, 2088, 3305, 2523, 2961, 2197, 2234)
...
conf_kbl <- conf_meta %>%
  ...
  left_join(., study_char.dat[ , c("refid", "study_l")], by = "refid") 
...
conf_kbl %>% 
  filter(arm == "Sug") %>% 
  select(confirm, study_l, design_f, arm_n, nmbagent, volatile, depth, rnmb_where, rnmb9_np, rnmb9_per) %>% 
  mutate(
  rnmb_bar = case_when(
...
kbl(
  ...
  pack_topnoind(., "≥0.9 confirmed", 1, 9) %>%
  pack_topnoind(., "Not confirmed", 10, 20) %>%
  pack_topnoind(., "Not stated", 21, 27) %>%
  footnote(
...
| Comparison                                  | RR (95% CI)       |
|:--------------------------------------------|:------------------|
| Confirmed vs. not confirmed                 | 0.07 (0.00, 0.56) |
|    Removing Alday 2019                      | 0.11 (0.00, 0.81) |
| Assuming all not stated were confirmed      | 0.09 (0.00, 0.60) |
| Assuming all not stated were not confirmed  | 0.13 (0.00, 1.07) |

</font>

<font size = 3> Risk ratios obtained as the ratio (median, lower and upper 0.025 quantiles) sampled from the means and standard deviations of the respective comparator incidence proportions (1E^6^ samples).</font>
...
kbl(
  ...
  column_spec(9, width = bar_width) %>% 
  pack_topnoind(., "≥0.9 confirmed", 1, 9) %>%
  pack_topnoind(., "Not confirmed", 10, 21) %>%
  pack_topnoind(., "Not stated", 22, 31) %>%
  footnote(
    general = "RCT: randomized controlled trial; NR Trial: nonrandomized trial; Prosp Cohort: prospective cohort; Retro Cohort: retrospective cohort; ROC: rocuronium; Vec: vecuronium; Var: various; Min: minimal; Mod: moderate; NS: not stated; PACU: post-anesthesia care unit.",
    alphabet = c("Extubation includes designated in OR."),
    general_title = "",
...
| Comparison                                  | RR (95% CI)       |
|:--------------------------------------------|:------------------|
| Confirmed vs. not confirmed                 | 0.13 (0.04, 0.32) |
| Assuming all not stated were confirmed      | 0.21 (0.06, 0.52) |
| Assuming all not stated were not confirmed  | 0.20 (0.06, 0.51) |
