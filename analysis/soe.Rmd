...
soe_kq1.dat <- readxl::read_xlsx("data/soe_2022-07-26.xlsx", range = "A1:Z8", sheet = "soe_kq1", col_types = col_types_nmb) %>%
  clean_names() %>%
  mutate(
...
soe_summary %>% 
  ...
  col.names = c("Outcome", "(pts)", " GRADE", "ACCF/AHA", "Importance^1^", "&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; Summary", "Result Detail")) %>% 
  ...
  pack_sub_bold(., "Adductor Pollicis vs. Corrugator Supercilii", 1, 1) %>% 
  pack_sub_bold(., "Adductor Pollicis vs. Orbicularis Oculi", 2, 2) %>% 
  pack_sub_bold(., "Adductor Pollicis vs. Masseter", 3, 3) %>% 
  pack_sub_bold(., "Adductor Pollicis vs. Flexor Hallucis Brevis", 4, 4) %>% 
  ...
  pack_sub_bold(., "Adductor Pollicis vs. Orbicularis Oculi", 7, 7) %>% 
...
col_types_nmb <- c("numeric", rep("text", 5), "numeric", rep("text", 19))
soe_kq3.dat <- readxl::read_xlsx("data/soe_2022-07-26.xlsx", range = "A1:Z23", sheet = "soe_kq3", col_types = col_types_nmb) %>%
  clean_names() %>%
  mutate(
    n_s = paste0(n_studies, " <br/>(", n_pts, ")"),
    n = paste0(n, outcome),
...
<font size = 4> Table `r table_n`. Strength of evidence for outcomes following neuromuscular monitoring (evidence obtained from RCTs unless noted). </font>
...
soe_summary %>%
  ...
  pack_topnoind(., "Quantitative vs. Qualitative or Clinical", 6, 8) %>%
  pack_topnoind(., "Quantitative vs. Qualitative", 9, 9) %>%
  pack_topnoind(., "Any monitoring vs. none", 10, 20) %>%
  footnote(
...
col_types_nmb <- c("numeric", rep("text", 5), "numeric", rep("text", 19))
soe_confirm.dat <- readxl::read_xlsx("data/soe_2022-07-26.xlsx", range = "A1:Z37", sheet = "soe_kq4", col_types = col_types_nmb) %>%
  clean_names() %>%
  mutate(
    n_s = paste0(n_studies, " <br/>(", n_pts, ")"),
    n = paste0(n, outcome),
  ...
  filter(str_detect(comparison, "conf_noconf"))
...
col_types_nmb <- c("numeric", rep("text", 5), "numeric", rep("text", 19))
soe_kq4.dat <- readxl::read_xlsx("data/soe_2022-07-26.xlsx", range = "A1:Z37", sheet = "soe_kq4", col_types = col_types_nmb) %>%
  clean_names() %>%
  mutate(
    n_s = paste0(n_studies, " <br/>(", n_pts, ")"),
    n = paste0(n, outcome),
...
<font size = 4> Table `r table_n`. Strength of evidence for reversal outcomes.</font>

```{r Kq4SOE, include = TRUE}
## (updated 2021/05/14 08:53) -----------------------
soe_summary %>% 
  filter(!str_detect(n, "99")) %>% 
  select(!c(comparison, n)) %>% 
  # slice(1,2,11) %>% 
  ...
  # pack_topnoind(., "Fully Paired", 1, 6) %>% 
  pack_sub_bold(., "Sugammadex vs. Neostigmine", 1, 3) %>% 
  pack_sub_bold(., "Cisatracurium and Atracurium", 28, 28) %>%
  # pack_sub_bold(., "Adductor Pollicis vs. Orbicularis Oculi", 2, 2) %>% 
  # pack_sub_bold(., "Adductor Pollicis vs. Masseter", 3, 3) %>% 
  # pack_sub_bold(., "Adductor Pollicis vs. Flexor Hallucis Brevis", 4, 4) %>% 
  # pack_sub_bold(., "Flexor Hallucis Brevis	vs. First Dorsal Interosseous", 5, 5) %>% 
