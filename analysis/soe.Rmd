...
col_types_nmb <- c("numeric", rep("text", 5), "numeric", rep("text", 19))
soe_kq3.dat <- readxl::read_xlsx("data/soe_111721_mac.xlsx", range = "A1:Z12", sheet = "soe_kq3", col_types = col_types_nmb) %>%
  clean_names() %>%
  mutate(
    n_s = paste0(n_studies, " <br/>(", n_pts, ")"),
    n = paste0(n, outcome),
...
soe_summary %>%
  ...
  pack_topnoind(., "Quantitative vs. Qualitative or Clinical", 6, 7) %>%
  pack_topnoind(., "Quantitative vs. Qualitative", 8, 8) %>%
  pack_topnoind(., "Any monitoring vs. none", 9, 9) %>%
  footnote(
...
col_types_nmb <- c("numeric", rep("text", 5), "numeric", rep("text", 19))
soe_kq4.dat <- readxl::read_xlsx("data/soe_111721_mac.xlsx", range = "A1:Z30", sheet = "soe_kq4", col_types = col_types_nmb) %>%
  clean_names() %>%
  mutate(
    n_s = paste0(n_studies, " <br/>(", n_pts, ")"),
    n = paste0(n, outcome),
