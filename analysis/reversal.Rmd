...
rnmb_dist_refids <- dichot.dat %>%
  ...
  filter(!is.na(rnmb)) %>%
  # filter(!is.na(rnmb), !refid %in% subgroup_refids) %>% 2022-05-03 to reconcile with ma spreadsheet
...
# add schaller per Thilen 2022-05-03 08:46
rnmb_dist_refids <- sort(c(rnmb_dist_refids, 2287))

## from spreadsheet ----------------------------------- (2022-02-22 14:32) @----
# rcts only from spreadsheet 2022/02/21 08:04 n=21 (checked same in excel sheet) 
# rnmb_ma.dat <- suppressMessages(read_csv("data/rnmb_read_011322.csv")) %>% (prior)
# DATA: rnmb_read_050322.csv ✓ 2022-07-23
rnmb_ma.dat <- suppressMessages(read_csv("data/rnmb_read_050322.csv")) %>% 
  clean_names() %>% 
  filter(!is.na(arm)) %>% 
  fill(refid, study) %>% 
...
study_arm_rev.dat <- study_arm.dat %>%
  ...
  mutate(
    ...
    arm = ifelse(str_detect(arm, "lacebo|placaebo|saline"), "Plac", arm),
...
temp_depth_rev <- study_arm_rev.dat %>% 
  ...
  mutate(check = 1) %>% 
  pivot_wider(id_cols = c(refid, study, arm), names_from = depth_rev_f, values_from = check, values_fn = sum) |> 
  mutate(across(mod:"deep:mod:shall", \(x) ifelse(is.na(x), " ", x)))
...
nmm.data <- BUGSnet::data.prep(
  # arm.data = comp_rct_arm_peds[comp_rct_arm_peds$refid < 3628,],
...
included_studies.dat <- study_char_rev.dat %>%
  mutate(
    ...
    surgs = str_replace(surgs, "Cr", "CR"),
    surgs = str_replace(surgs, "Headneck", "ENT"),
    surgs = str_sub(surgs, 1, nchar(surgs) - 2),
    surgs = ifelse(surg_other_desc == "laparoscopic", "Lap", surgs),
    surgs = ifelse(surg_other_desc == "surgical procedures of the periphery", "Elective", surgs),
...
included_studies.dat %>% 
  ...
  pack_top(., "Adult",                      1, 183) %>%
  pack_sub(., "RCT",                        1, 131) %>% 
  pack_sub(., "Before-After/Time Series", 130, 131) %>% 
  pack_sub(., "Nonrandomized Trial",      132, 142) %>% 
  pack_sub(., "Prospective Cohort",       143, 167) %>% 
  pack_sub(., "Retrospective Cohort",     168, 183) %>% 
  pack_top(., "All ages",                 184, 191) %>% 
  pack_sub(., "RCT",                      184, 187) %>% 
  pack_sub(., "Retrospective Cohort",     188, 191) %>% 
  pack_top(., "Pediatric",                192, 210) %>%
  pack_sub(., "RCT",                      192, 205) %>% 
  pack_sub(., "Prospective Cohort",       206, 209) %>% 
  pack_sub(., "Case-control",             210, 210) %>% 
  footnote(
    general = "RCT: randomized controlled trial; CR: colorectal; Car: cardiac; GI: gastrointestinal; Lap: laparascopic; Ortho: orthopedic; Abdom: abdominal; ENT: otolaryngology (ear, nose, and throat); Gyn: gynecologic; Urol: urologic; Neuro: neurological; Hep: hepatic; Thor: thoracic; Oralmax: oral maxillofacial.",
    alphabet = c(
...
## from distiller; not used --------------------------- (2022-07-21 10:52) @----
# note comes from distiller; not used
temp <- rnmb.dat %>%
  mutate(
    # dose_comb = neo_dose,
    dose_comb = case_when(
  ...
  rename_with(.fn = ~ str_extract(.x, "\\w{3}"), .cols = cisatracurium:mivacurium) %>% 
  ...
  select(refid, design, study_l, study, arm_n, arm, depth_rev_f, deep:ns_depth, dose_comb, roc, vec, var, tiva, volatile, anesth_ns, rnmb_where, rnmb_dev, mon_cat_f)
  
## rnmbData_ma ---------------------------------------- (2022-02-28 08:51) @----
# DATA: rnmb_read_050322.csv ✓ 2022-07-23
rnmb_ma.dat <- read_csv("data/rnmb_read_050322.csv") %>% 
  clean_names() %>% 
  filter(!is.na(arm)) %>% 
  fill(refid, study) %>% 
  remove_empty(which = "cols") %>% 
...
rnmb_ma.dat %>% 
  ...
  kbl(
    booktabs = T, align = c("llccccccccccccc"), escape = FALSE, # format = "latex",
    col.names = c("Study", " N", "Agent", "mg/kg; <u>mg</u>", "Roc", "Vec", "Cis", "TIVA", "Volat", "NS", "AMG", "EMG", "KMG", "Clin", "NS")
  ) %>%
  ...
  pack_top(., "RCT", 1, 35, indent_tf = FALSE) %>% 
  pack_top(., "Nonrandomized Trials", 36, 45, indent_tf = FALSE) %>% 
  pack_top(., "Prospective Cohort", 46, 60, indent_tf = FALSE) %>% 
  pack_top(., "Retrospective Cohort", 61, 64, indent_tf = FALSE) %>% 
...
```{r rnmbKblPrimaryDistiller, eval = FALSE, include = FALSE}
## not used ------------------------------------------- (2022-07-22 09:23) @----
## rnmbKbl -------------------------------------------- (2022-04-02 20:02) @----
# distiller 2022-05-04 does not include Schaller
...
temp %>%
  ...
  column_spec(15, width = or_width, popover = "RR (95% CI)") %>%
  pack_top(., "RCT", 1, 35, indent_tf = FALSE) %>% 
  pack_top(., "Nonrandomized Trials", 36, 45, indent_tf = FALSE) %>% 
  pack_top(., "Prospective Cohort", 46, 60, indent_tf = FALSE) %>% 
  pack_top(., "Retrospective Cohort", 61, 64, indent_tf = FALSE) %>% 
...
rnmb_meta.dat <- left_join(rnmb_meta.dat, temp1, by = c("refid", "study_orig")) %>% 
  select(-c(Neo, Sug)) %>% 
  mutate(depth_f = factor(depth, levels = c("Deep", "Moderate:Deep", "Moderate", "Minimal", "Not specified"))) |> 
  arrange(depth_f, year)
...
<font size = 4> Figure `r figure_n`. Pooled residual neuromuscular blockade (TOFR \< 0.9) from randomized trials comparing sugammadex with neostigmine by depth of block. </font>

```{r rnmbMetaRctByDepth, fig.width = 10, fig.height = 8}
# TODO: fig.height
## rnmbMetaRctByVolatile -----------------------------+ (2022-07-23 15:58) @----
...
Risk difference -21.6% (95% CI, -33.8% to -9.4%); n.b., includes 10 studies (n = 1451), while RR estimate 8 studies (n = 1143). Too few studies to examine small study effects. Schaller 2010 included based on range of recovery times.

<br/>

```{r smallStudyEffectsRnmb, eval = FALSE}
## smallStudyEffects ---------------------------------- (2022-07-23 15:58) @----
temp_meta <- rnmb_meta
...
<font size = 4> Figure `r figure_n`. Pooled residual neuromuscular blockade (TOFR \< 0.7) from randomized trials comparing sugammadex with neostigmine by depth of block. </font>

```{r rnmbMetaRct0.7, fig.width = 10, fig.height = 4.5}
# TODO: fig.height
## rnmbMetaData0.7 ------------------------------------ (2022-07-23 15:57) @----
rnmb_meta.dat <- temp %>%
  filter(design == "rct", arm %in% c("Neo", "Sug"), age == "Adult") %>%
  mutate(
    depth = case_when(
...
Risk difference -14.4% (95% CI, -22.5% to -6.3%). Too few studies to examine small study effects.
...
## rnmbDescrPedsKbl ----------------------------------+ (2022-07-23 15:57) @----
# rnmb.dat %>% 
rnmb_ma.dat %>% 
  filter(age == "Pediatric") %>%
  select(study_l, arm:ns_mon) %>% 
  ...
  select(-c(emg:ns_mon)) |> 
  kbl(
    booktabs = T, align = c("llcccccccc"), escape = FALSE, # format = "latex",
    col.names = c("Study", " N", "Agent", "mg/kg", "Roc", "Vec", "Cis", "TIVA", "Inh", "NS", "AMG")
  ) %>%
  add_header_above(c(" " = 2, "Reversal" = 2, "NMBA" = 3, "Anesthetic" = 3, "Monitor" =  1), line = TRUE, bold = TRUE) %>%
  row_spec(0, bold = TRUE) %>%
  kable_styling(full_width = FALSE, bootstrap_options = opt_boot, position = "left") %>%
  kable_classic(full_width = FALSE, html_font = opt_font, position = "left", "hover") %>% 
  column_spec(1, width = study_width) %>%
  ...
  pack_top(., "Prospective Cohort", 3, 4, indent_tf = FALSE) %>% 
    footnote(
    general = "NMBA: neuromuscular blocking agent; Neo: neostigmine; Sug: sugammadex; Cis: cisatracurium; Spont: spontaneous; Roc: rocuronium; Vec: vecuronium; TIVA: total intravenous anesthesia; Inh: inhalation; NS: not specified; AMG: acceleromyography.",
    # alphabet = c(""),
    general_title = "",
    footnote_as_chunk = FALSE
  )  
...
```{r timeData}
## verify all refids not run  ------------------------- (2022-07-23 15:56) @----
...
## data ----------------------------------------------- (2022-07-23 15:56) @----
# DATA: recoveryTime_2022-07-21.csv ✓ 2022-07-23
time.dat <- suppressWarnings(read_csv("data/recovery_time_2022-07-22.csv", col_types = c("cccccncccccccccccccncccccc"))) %>%  
  clean_names() %>% # get volatiles from study_arm.dat
  remove_empty(which = c("cols", "rows")) %>% # why new names
  fill(refid, study, arm, mg_kg, n, depth) %>% 
  mutate(id = row_number()) %>% 
  relocate(id, .before = refid) %>% 
  mutate(refid = as.numeric(refid),
    arm = ifelse(arm == "Plac", "Placebo", arm), 
    arm = str_replace(arm, " \\(CBW\\)", ""),
    arm_merge = str_extract(arm, "\\w{3,7}")
  ) |> 
  filter(!is.na(refid)) |> 
  relocate(arm_merge, .after = arm) |> 
  filter(!refid %in% c(2514, 3338, 2502, 2918))
...
# temp_write <- tab_lst(time.dat, arm)
# writexl::write_xlsx(temp_write, "/Users/mgrant/Desktop/temp_write.xlsx")
# write_csv(as.data.frame(time.dat), "/Users/mgrant/Desktop/temp_write.csv", na = "")

## add study_char.dat data ---------------------------- (2022-07-23 15:56) @----
temp <- study_char.dat %>% select(refid, study_l, age, design, year)
...
temp <- study_arm.dat %>% 
  ...
  mutate(
    volatiles = if_any(c(an_isoflurane, an_sevoflurane, an_desf, an_hal), ~ !is.na(.))) |> 
  # select(refid, study, nmbagent, study_arm_k, gen_type, volatiles, an_isoflurane, an_sevoflurane, an_desf, an_hal) %>% 
  select(refid, nmbagent, study_arm_k, gen_type, volatiles) %>% 
  mutate(
  ...
  rename(arm_merge = arm) |> 
  select(-study_arm_k)

time.dat <- left_join(time.dat, temp, by = c("refid", "arm_merge")) %>% 
  relocate(c(nmbagent, gen_type), .before = amg) %>% 
  group_by(id) %>% 
  slice(1) %>% 
  ungroup()

# studies without b/w drug comparison
single_comp_refids <- time.dat |> 
  mutate(arm_new = str_extract(arm, "\\b\\w+\\b")) |> 
  relocate(arm_new, .after = arm) |> 
  group_by(study) |> 
  filter(n_distinct(arm_new) == 1) |> 
  ungroup() |> 
  pull(refid) |> 
  unique()

time.dat <- time.dat |> 
  filter(!refid %in% single_comp_refids) 

# removed studies 
single_comp_studies <- time.dat |> 
  filter(refid %in% single_comp_refids) |> 
  select(study, design, age, arm)
```
...
```{r timeDescrKblRct}
## timeDescrKblRct -----------------------------------+ (2022-07-23 15:56) @----
# select only unique arms for studies reporting 0.8 and 0.9 and 0.1
time_9 <- time.dat %>%
  filter(tofr == 0.9)
...
time_8 <- time.dat %>%
  filter(tofr == 0.8) %>% 
  # only no time to tofr 0.9
  filter(!refid %in%  c(time_9$refid, time_1$refid))

time_kbl_adult <- bind_rows(time_9, time_8, time_1) %>% 
  filter(age == "Adult") %>%
  # filter(refid == 2000) %>% 
  mutate(
    ...
    tiva = ifelse(str_detect(gen_type, "tiva"), "&times;", NA),
    volatiles = ifelse(volatiles == TRUE, "&times;", NA),
  ...
  mutate(
    ...
    depth_lowest_f = factor(depth_lowest_f, levels = c("Deep", "Moderate", "Shallow", "Minimal", "End of Surgery", "Other"))) %>% 
  ungroup() %>% 
...
time_kbl_adult %>%
  ...
  pack_topnoind(., "RCT", 1, 100) %>% 
  # pack_topnoind(., "Nonrandomized Trial", 107, 112) %>%
  # pack_topnoind(., "Prospective Cohort", 113, 114) %>%
...
```{r timeResultsData}
## timeResultsData ------------------------------------ (2022-07-23 15:56) @----
time_result_adult <- time.dat %>%
  ...
  mutate(
    ...
    iqr_u =  str_extract(med_iqr, "\\d{1,3}\\.\\d{1,2}\\)"),
    iqr_u = as.numeric(str_replace(iqr_u, "\\)", "")),
    range_l = as.numeric(str_extract(range, "^\\d{1,2}\\.?\\d{0,2}")),
    range_u = as.numeric(str_extract(range, "\\d{1,3}\\.?\\d{0,2}$")),
...
# write_csv(time_result_adult, "/Users/mgrant/Desktop/time_result_adult.csv", na = "")
```
...
## timeResultsRctAdult -------------------------------+ (2022-07-23 15:55) @----
...
# add Pongracz 2013 tofr1 shallow
# tofr1_study <- temp |> 
#   filter(tofr == 1.0) |> 
#   pull(study_l)
# 
...
#   pull(study_l)
# 
# tofr1_study[tofr1_study %in% tofr9_study]

# temp %>%
#   filter(tofr == 0.9 | str_detect(study_l, "Pongracz 2013")) %>%
#   arrange(depth_f) %>%
#   select(depth_f) %>%
#   group_by(depth_f) %>%
#   tally() %>%
...
temp %>%
  ...
  pack_top(., "Deep Block^a^", 1, 10, indent_tf = FALSE) %>% 
  pack_top(., "Moderate Block^b^", 11, 52, indent_tf = FALSE) %>% 
  pack_top(., "Shallow Block^a^", 53, 72, indent_tf = FALSE) %>% 
  pack_top(., "Minimal^a^", 73, 77, indent_tf = FALSE) %>% 
  pack_top(., "End of Surgery^a^", 78, 91, indent_tf = FALSE) %>% 
  pack_top(., "Other", 92, 93, indent_tf = FALSE) %>% 
  footnote(
    ...
    alphabet = c(
      ...
      "Time to TOFR 1.0 (did not report to 0.9) Pongracz."
    ),
    general_title = "",
    footnote_as_chunk = FALSE
  )
...
## metaTime9 data ------------------------------------- (2022-07-23 15:55) @----
meta_time.dat <- time_result_adult %>%
  # Gaszynski 2012 remove (CBW)
  mutate(
    arm = str_replace(arm, " \\(CBW\\)", "")
  ) |> 
  filter(design == "rct", (tofr == 0.9 | str_detect(study_l, "Pongracz 2013"))) %>% 
  ...
  ungroup() |> 
  # fix He 2022
  filter(!(study == "He 2022" & med == 3.4)) |> 
  mutate(
    n = ifelse(refid == 7408 & arm == "Neo", 22, n),
    mean = ifelse(refid == 7408 & arm == "Neo", 3.55, mean),
    sd = ifelse(refid == 7408 & arm == "Neo", 1.33, sd),
    med = ifelse(refid == 7408 & arm == "Neo", NA, med),
    range_l = ifelse(refid == 7408 & arm == "Neo", NA, range_l),
    range_u = ifelse(refid == 7408 & arm == "Neo", NA, range_u),
    # schepens 2019 range 2 to 2 assume sd of 0.6 based on other sug arms studies with reversal time < 3 min
    sd = ifelse(refid == 3353 & arm == "Sug", 0.85, sd),
    mean = ifelse(refid == 3353 & arm == "Sug", 2, mean),
  ) 

# meta_time.dat |> 
#   filter(mean < 3, depth_f == "Moderate", arm == "Sug") |> 
#   select(sd) |> 
#   summarize(sd = mean(sd, na.rm = TRUE))
# 
# 
# temp <- meta_time.dat |> 
#   filter(range_l - range_u >= 0)


# arms to combine 2022-07-22 only He 
# meta_time.dat |> 
#   group_by(study) |> 
#   filter(n() > 2) |> 
#   ungroup()
# combine_contin(11, 11, 3.7, 3.4, 1.45, 1.26)

# verify spont excluded correct 2022-07-22
# meta_time.dat |> 
#   group_by(study) |> 
#   filter(n()  == 1 ) |> 
#   ungroup()
...
meta_time.dat <- meta_time.dat %>% 
  ...
  # pivot_wider(names_from = "arm", values_from = depth_f:range_u) %>% 
  # filter(depth_f_Neo != depth_f_Sug) |> # to verify arms the same depth; ok 2022-05-05 10:02
  pivot_wider(names_from = "arm", values_from = n:range_u) %>%
  mutate(volatiles = ifelse(volatiles ==  TRUE, "Volatile anesthetic", "No volatiles"))

## meta analysis -------------------------------------- (2022-07-23 15:55) @----
temp <- metacont(
  n.e = n_Sug, 
  n.c = n_Neo,
  # common =  TRUE,
...
Note estimate from imputed means and standard deviations using median and range.

<a id="revFig8"></a>

<br/>

<font size = 4> Figure `r figure_n`. Difference in time to recovery in minutes (TOFR \> 0.9) in studies reporting from end of surgery to reversal in RCTs enrolling adults. </font>
...
```{r timeDescrKblObs, eval = FALSE}
## timeDescrKblRct ------------------------------------ (2022-07-23 15:54) @----
# time.dat %>%
#   filter(design != "rct") %>%
#   arrange(design, study_l) %>%
#   select(study_l, refid, age, design) %>%
...
```{r timeResultsObsAdults, eval = FALSE}
## timeResultsRctAdults ------------------------------ (2022-07-23 15:54) @----
temp <- time_result_adult %>%
  filter(design != "rct") %>%
  mutate(
    mean_med = coalesce(mean, med),
...
temp %>%
  ...
  pack_top(., "Other/End of Surgery", 17, 24, indent_tf = FALSE) %>% 
...
# DATA: rev_agent_summary_2022-07-22.xlsx ✓ 2022-07-23
adverse.dat <- suppressWarnings(readxl::read_xlsx("data/rev_agent_summary_2022-07-22.xlsx",
  sheet = "Adverse Events", range = "A4:N94",
...
popc.dat <- suppressWarnings(readxl::read_xlsx("data/rev_agent_summary_2022-07-22.xlsx",
  sheet = "POPC", range = "A4:U143",
...
ponv.dat <- suppressWarnings(readxl::read_xlsx("data/rev_agent_summary_2022-07-22.xlsx",
  sheet = "PONV", range = "A5:K121",
...
adverse.dat %>% 
 ...
 kbl(
  ...
  pack_topnoind(., "Adult", 1, 6) %>% 
  pack_rows(., "RCT", 1, 4, indent = FALSE) %>% 
  pack_rows(., "Retrospective Cohort", 5, 6, indent = FALSE) %>% 
  pack_sub_bold(., "All ages", 7, 12) %>% 
  pack_rows(., "Retrospective Cohort", 7, 12, indent = FALSE) %>% 
  # pack_rows(., "Case Control", 9, 10) %>% 
  footnote(general = "IBW: ideal body weight; RCT: randomized controlled trial.",
  #        number = c("● Limited, ●● Important, ●●● Critical."),
           general_title = "",
           footnote_as_chunk = FALSE)
...
Anaphylaxis reported 0.3 per 10,0000 (95% CI, 0.10-0.93; 5 studies, 99375 patients); in patients receiving sugammadex (7 studies, 104777 patients) the pooled incidence was 1.4 per 10,000 (95% CI, 0.7, 3.1).

```{r anaphylaxisPooled, eval = FALSE}
## anaphylaxisPooled ---------------------------------- (2022-05-03 07:22) @----
anaphylaxis <- adverse.dat %>% 
  filter(age %in% c("Adult", "All ages")) %>% 
  # filter(str_detect(arm, "Neo")) |> 
  mutate(year = str_extract(study, "\\d{4}"),
         # anaph_np = n_percent_dig(anaph_n, anaph_n/n * 100, 2),
         # design = factor(design, levels = c("prospective", "retro", "case control"))
         ) %>% 
...
adverse.dat %>%
  ...
  pack_rows(., "RCT", 1, 48, indent = FALSE) %>%
  pack_rows(., "Nonrandomized Trial", 49, 50, indent = FALSE) %>%
  pack_rows(., "Retrospective Cohort", 51, 52, indent = FALSE) %>%
  # pack_rows(., "Case Control", 9, 10) %>%
  footnote(
...
<font size = 4> Figure `r figure_n`. Bradycardia pooled results from RCTs comparing sugammadex with neostigmine. </font>

```{r pooledBrady, fig.width = 10, fig.height = 8}
## pooledBrady --------------------------------------- (2022/04/02 10:21) @----
brady_meta.dat <- adverse.dat %>%
  filter(age == "Adult", design == "rct") %>%
  filter(arm %in% c("Neo", "Sug", "Neo LBW", "Sug TBW", "Neo (no Mg)", "Spont (no Mg)", "Neo (>65 y)", "Sug (>65 y)")) %>%
  filter(!is.na(brady_n)) %>%
  mutate(arm = str_replace(arm, " .*", "")) %>%
  group_by(study) %>%
  filter(n() > 1) %>%
  ungroup() %>%
  filter(!refid %in% c(2004, 1155)) %>% # Suy 2007 not sug:neo comparison
  # select(refid, study, arm, brady_n, n, mg_kg) %>%
  select(refid, study, arm, brady_n, n, mg_kg, antichol) %>%
  pivot_wider(names_from = arm, values_from = c(brady_n, n, mg_kg, antichol)) %>% 
  ...
  mutate(antichol_Neo = ifelse(!antichol_Neo %in% c("Glyco", "Atropine"), "Not Reported", antichol_Neo),
         antichol_Neo = ifelse(antichol_Neo == "Glyco", "Glycopyrrolate", antichol_Neo))
...
brady_meta <- metabin(brady_n_Sug, n_Sug, brady_n_Neo, n_Neo,  
  ...
  # allstudies = FALSE,
  hakn = TRUE,
  subgroup = antichol_Neo,
  # subset = antichol_Neo != "Not Reported",
  # subset = antichol_Neo == "Glycopyrrolate",
  prediction = TRUE)
  # subset = !study_orig %in% c("Wu 2014 (Chinese)", "Wu 2014 (Caucasian)"),
  #subgroup = depth)
...
## neoRate -------------------------------------------- (2022-07-23 12:32) @----
neo_meta.dat <- adverse.dat %>%
  filter(age == "Adult", design == "rct") |> 
  filter(str_detect(arm, "Neo")) |> 
  filter(!is.na(brady_n)) |> 
  filter(!(study == "Fuchs-Buder 1999" & brady_n == 5)) |> # combine arms below
  mutate(arm = str_replace(arm, " .*", ""),
         n = ifelse(study == "Fuchs-Buder 1999", 24, n),
         brady_n = ifelse(study == "Fuchs-Buder 1999", 9, brady_n),
         upper_dose = str_extract(mg_kg,"\\d\\.\\d{2}$")) |> 
  select(refid, study, brady_n, n, mg_kg, upper_dose) |> 
  arrange(upper_dose)

# anticholinergic and dose
antichol_dat <- study_arm.dat |> 
  filter(arm_comp_reversal == "neo", refid %in% neo_meta.dat$refid) |> 
  mutate(
    antichol_unit = ifelse(refid == 3098, "mg_per_kg", antichol_unit),
    antichol_unit = ifelse(antichol_unit == "mg_per_kg", "mg/kg", antichol_unit),
    antichol = case_when(
    !is.na(atrop) ~ "Atropine",
    !is.na(glyco) ~ "Glycopyrrolate",
    TRUE ~ "NS"
  ),
  n = arm_n,
  dose = paste(antichol_dose, antichol_unit),
  dose = ifelse(dose == "NA NA", " ", dose)) |> 
  select(refid, antichol, dose)

neo_meta_dat <- left_join(neo_meta.dat, antichol_dat, by = "refid") |> 
  select(-refid)

neo_brady_meta <- metaprop(brady_n, n,
  data = neo_meta.dat,
  common = FALSE,
  studlab = study,
)
...
forest(neo_brady_meta,
  rightcols = c("effect", "ci", "mg_kg"), # "mg", "dose"),
  rightlabs = c(" ", "95% CI    " , "Dose (mg/kg)"), # "Antichol", "Dose"),
  studlab = TRUE,
  common = FALSE,
  digits = 1,
  digits.tau2 = 1,
...
adverse.dat %>%
  ...
  pack_rows(., "RCT", 1, 12, indent = FALSE)
...
tachy_meta.dat <- adverse.dat %>%
  ...
  select(refid, study, arm, tachy_n, n, mg_kg, antichol) %>%
  pivot_wider(names_from = arm, values_from = c(tachy_n, n, mg_kg, antichol)) %>%
  # means when range neo
  mutate(
    mg_kg_Neo = case_when(
      study == "Schaller 2010" ~ "0.02",
  ...
  mutate(
    antichol_Neo = ifelse(!antichol_Neo %in% c("Glyco", "Atropine"), "Not Reported", antichol_Neo),
    antichol_Neo = ifelse(antichol_Neo == "Glyco", "Glycopyrrolate", antichol_Neo)
  )
  # add reversal depth; need to use only selected arm
  # left_join(., study_arm_rev.dat %>% select(refid, depth_rev_f), by = "refid")

tachy_meta <- metabin(tachy_n_Sug, n_Sug, tachy_n_Neo, n_Neo, 
  data = tachy_meta.dat,
  studlab = study,
  sm = "RD",
  method = "MH",
...
Risk difference for sugammadex vs. neostigmine -1.1% (95% CI, -4.1% to 1.9%).

```{r arrhythmiaMeta, eval = FALSE}
## arrhythmiaMeta ------------------------------------- (2022-07-23 07:22) @----
arrhyth <- adverse.dat %>% 
  filter(age %in% c("Adult", "All ages")) %>% 
  mutate(year = str_extract(study, "\\d{4}")) %>% 
  filter(!is.na(arrhyth_n)) %>%
  mutate(arm = str_extract(arm, "^\\w{3}")) |> 
  # arrange(age, design, year, refid) %>% 
  select(study, arm, n, arrhyth_n) %>% 
  pivot_wider(names_from = arm, values_from = c(arrhyth_n, n))

arrhythmia_meta <- metabin(
  arrhyth_n_Sug, n_Sug,
  arrhyth_n_Neo, n_Neo,
  pscale = 100,
  sm = "RD",
...
ponv.dat %>%
  ...
  column_spec(5, width = "11em") %>%
  column_spec(6, width = bar_width) %>%
  pack_rows(., "RCT", 1, 34, indent = FALSE) %>%
  pack_rows(., "Nonrandomized Trial", 35, 36, indent = FALSE) %>%
  pack_rows(., "Retrospective Cohort", 37, 44, indent = FALSE) %>%
  footnote(
    # general = "IBW: ideal body weight.",
    #        number = c("● Limited, ●● Important, ●●● Critical."),
    general_title = "",
...
net_table <- BUGSnet::net.tab(temp,
  outcome = "ponv_n",
  N = "arm_n",
...
## ponvMetaForest ------------------------------------- (2022-07-23 14:26) @----
library(netmeta)
pairs.dat <- pairwise(
  treat = arm,
  event = ponv_n,
...
## ponvNodeSplit -------------------------------------- (2022-07-23 14:28) @----
...
```{r ponvCinema, eval = FALSE, include = FALSE, echo = FALSE}
## cinema --------------------------------------------- (2022-07-23 15:24) @----
# rob file
ponv_nma_rob <- read_csv("data/rob_overall.csv") |> 
  filter(refid %in% nma_ponv$refid)

ponv_cinema <- nma_ponv %>%
  select(refid, study, arm, arm_n, ponv_n) %>%
  group_by(study) %>%
  mutate(id = cur_group_id(), indirectness = 2) %>%
  ungroup()

temp <- left_join(ponv_cinema, ponv_nma_rob, by = "refid") %>%
  rename(t = arm) %>%
  ...
  select(study, id, t, arm_n, ponv_n, rob, indirectness)

write_csv(temp, "data/ponv_cinema.csv")
```
...
```{r nauseaMeta}
## nauseanmaData ---------------------------------------- (2022-07-23 15:33) @----
library(netmeta)
nma_nausea <- ponv.dat %>%
...
## nauseaMetaForest ----------------------------------- (2022-07-23 15:33) @----
pairs.dat <- pairwise(
  treat = arm,
  event = nausea_n,
  n = arm_n,
...
temp <- netsplit(nausea_netmeta_nma,
  order = trts
) 
...
```{r nauseaCinema, eval = FALSE, include = FALSE, echo = FALSE}
## cinema --------------------------------------------- (2022-07-23 15:24) @----
# rob file
nausea_nma_rob <- read_csv("data/rob_overall.csv") |> 
  filter(refid %in% nma_nausea$refid)

nausea_cinema <- nma_nausea %>%
  select(refid, study, arm, arm_n, nausea_n) %>%
  group_by(study) %>%
  mutate(id = cur_group_id(), indirectness = 2) %>%
  ungroup()

temp <- left_join(nausea_cinema, nausea_nma_rob, by = "refid") %>%
  rename(t = arm) %>%
  mutate(rob = case_when(
    overall == "yes" ~ 1,
    overall == "unclear" ~ 2,
  ...
  select(study, id, t, arm_n, nausea_n, rob, indirectness)

write_csv(temp, "data/nausea_cinema.csv")
```
...
ponv.dat %>%
  ...
  column_spec(5, width = "11em") %>%
  column_spec(6, width = bar_width) %>%
  pack_rows(., "RCT", 1, 46, indent = FALSE) %>%
  pack_rows(., "Nonrandomized Trial", 47, 48, indent = FALSE) %>%
  pack_rows(., "Retrospective Cohort", 49, 52, indent = FALSE) %>%
  # pack_rows(., "Case Control", 9, 10) %>%
  footnote(
...
## vomitMetaForest ------------------------------------ (2022-07-23 15:50) @----
pairs.dat <- pairwise(
  treat = arm,
  event = vomit_n,
  n = arm_n,
...
```{r vomitCinema, eval = FALSE, include = FALSE, echo = FALSE}
## cinema --------------------------------------------- (2022-07-23 15:24) @----
# rob file
vomit_nma_rob <- read_csv("data/rob_overall.csv") |> 
  filter(refid %in% nma_vomit$refid)

vomit_cinema <- nma_vomit %>%
  select(refid, study, arm, arm_n, vomit_n) %>%
  group_by(study) %>%
  mutate(id = cur_group_id(), indirectness = 2) %>%
  ungroup()

temp <- left_join(vomit_cinema, vomit_nma_rob, by = "refid") %>%
  rename(t = arm) %>%
  mutate(rob = case_when(
    overall == "yes" ~ 1,
    overall == "unclear" ~ 2,
  ...
  select(study, id, t, arm_n, vomit_n, rob, indirectness)

write_csv(temp, "data/vomit_cinema.csv")
```
...
hypox_dat %>%
  ...
  pack_topnoind(., "    SaO^2^ ≤90%", 1, 22) %>%
  pack_sub_bold(., "RCT", 1, 13) %>%
  pack_sub_bold(., "Interrupted Time Series", 14, 15) %>%
  pack_sub_bold(., "Nonrandomized Trials", 16, 17) %>%
  pack_sub_bold(., "Prospective Cohort", 18, 22) %>%
  pack_topnoind(., "    SaO^2^ 90-95%", 23, 42) %>%
  pack_sub_bold(., "RCT", 23, 40) %>%
  pack_sub_bold(., "Retrospective Cohort", 41, 42) %>%
  pack_topnoind(., "    SaO^2^ Not Specfified", 43, 44) %>%
  pack_sub_bold(., "RCT", 43, 44) %>%
  footnote(
...
composite <- popc.dat %>%
  ...
  mutate(
    ...
    composite_np = ifelse(study == "Li 2021", "see below", composite_np)
  ) %>%
...
composite %>% 
  ...
  pack_topnoind(., "Prospective Cohort", 24, 34) %>%
  pack_topnoind(., "Retrospective Cohort", 35, 48) %>%
  # result_pack(., "")
  footnote(
    general = "NS: not specified; mod: moderate; endsurg: end of surgery; shall: shallow; min: minimal; oth: other ",
    # number = c(""),
...
# TODO: xlsx file
comp_defn <- readxl::read_xlsx("data/popc_rev_agent_2022-07-23.xlsx", sheet = "comp_defn", range = "A1:C14") %>% 
...
comp_nrsi <- tribble(
  ...
  "Cho 2021",        log(0.99), sd_confint(log(0.31), log(3.15)), "nrsi",
  "Cheng 2022",      log(0.52), sd_confint(log(0.32), log(0.85)), "nrsi",
)
...
pneumonia %>% 
  ...
  pack_topnoind(., "Retrospective Cohort", 16, 25) %>%
  pack_topnoind(., "Case-Control", 26, 27) %>%
  footnote(
    general = "NS: not specified; mod: moderate; endsurg: end of surgery; shall: shallow; min: minimal; oth: other ",
    # number = c(""),
    general_title = "",
...
pneum_nrsi <- tribble(
  ...
  "Cheng 2022",     log(0.11), sd_confint(log(0.01), log(0.91)), "nrsi",
)
...
Results from 3 nonrandomized studies adjusted for potential confounding (Li 2021 and Yu 2021 propensity scores, Kheterpal 2020 matched cohort design).<br/> Pooled risk difference comparing sugammadex with neostigmine in for RCTs -1.6% (95% CI, -4.3 to 1.1; I^2^ = 20%, 𝜏= 1.4%.
...
popc.dat |>
  ...
  pack_topnoind(., "Retrospective Cohort", 5, 12) %>%
  result_pack(., "RR 1.97 (95% CI, 0.67-5.78)", 7, padding = 62) %>% 
  result_pack(., "OR 0.45; 95% CI, 0.37-0.56)", 11, padding = 62) %>% 
    pack_topnoind(., "Case-Control", 13, 14) %>%
  footnote(
    general = "NS: not specified; mod: moderate; endsurg: end of surgery; shall: shallow; min: minimal; oth: other ",
    # number = c(""),
    general_title = "",
...
bronchospasm %>% 
  ...
  pack_topnoind(., "Retrospective Cohort", 9, 10) %>%
  footnote(
    general = "NS: not specified; mod: moderate; endsurg: end of surgery; shall: shallow; min: minimal; oth: other ",
    # number = c(""),
    general_title = "",
...
reintub <- popc.dat %>%
  ...
  filter(refid %notin% c(3574, 2304, 7032, 7124)) |> 
  mutate(
    year = str_extract(study, "\\d{4}"),
    ...
    arm_c = case_when(
      ...
      design_up = as.character(design),
      design_up = ifelse(study == "Llaurado 2014", "Prospective Historical Control", design_up)
  ) %>%
  filter(age == "Adult") %>%
...
reintub %>% 
  ...
  column_spec(7, width = bar_width) %>%
  pack_topnoind(., "RCT", 1, 10) %>%
  pack_topnoind(., "Interrupted Time Series", 11, 12) %>%
  pack_topnoind(., "Non-randomized Trial", 13, 14) %>%
  pack_topnoind(., "Prospective Cohort", 15, 17) %>%
  pack_topnoind(., "Retrospective Cohort", 18, 19) %>%
  pack_topnoind(., "Prospective Historical Control", 20, 21) %>%
  footnote(
    general = "NS: not specified; mod: moderate; endsurg: end of surgery; shall: shallow; min: minimal; oth: other ",
    # number = c(""),
    general_title = "",
...
<font size = 4> Figure `r figure_n`. Pooled difference in risk of postoperative reintubation in randomized and nonrandomized studies. </font>

```{r reintubMeta, fig.width = 10, fig.height = 6}
...
reintub_meta_dat <- reintub %>% 
  ...
  mutate(design = ifelse(design %in%  "rct", "RCT", "Observational"))
...
reintub_rct_meta <- metabin(reintub_n_Sug, arm_n_Sug, reintub_n_Neo, arm_n_Neo,
  ...
  pscale = 100,
  subgroup = design,
  # subset = design !== "RCT" & study != "Martinez-Ubieto 2016",
)
...
<font size = 4> Table `r table_n`. Rates of reparalysis in studies including comparison of sugammadex and neostigmine. </font>

```{r recurar}
## recurar -------------------------------------------- (2022-05-05 14:04) @----
# TODO: csv file
recur.dat <- suppressMessages(read_csv("data/weakness_2022-07-25.csv")) |> 
  clean_names() %>%
  remove_empty(which = "cols") %>%
  fill(refid:study) %>%
  mutate(refid = suppressWarnings(as.numeric(refid)),
         year = str_extract(study, "\\d{4}")) %>%
  remove_empty(which = "rows") %>%
  mutate(
   recur_n = as.numeric(str_extract(recurarization, "^\\d*")),
   study = ifelse(study == "Abdulatif 2018" & str_detect(arm, "non-hepatic"), "Abdulatif 2018 (non-hepatic)", study),
   study = ifelse(study == "Abdulatif 2018" & str_detect(arm, "hepatic"), "Abdulatif 2018 (hepatic)", study),
  ) %>%
  # relocate(recur_n, .after = n)
  filter(!is.na(recur_n)) |> 
  select(refid, study, year, arm:volatiles, deep:min, recur_n) |> 
  left_join(study_ident[, c("refid", "study_l", "age", "design")], by = "refid") %>% 
  relocate(study_l:design, .before = study)

# studies including neostigmine sugammadex comparison
compare_refids <- recur.dat |> 
  mutate(
    arm_sort = str_extract(arm, "\\w{3}")
  ) |> 
  # select(study, arm, arm_sort) |> 
  group_by(study) |> 
  filter(any(arm_sort == "Sug" & any(arm_sort == "Neo"))) |> 
  ungroup() |> 
  pull(refid)

recur.dat <- recur.dat |> 
  mutate(arm_sort = str_extract(arm, "\\(.*\\)")) |> 
  filter(refid %in% compare_refids) |> 
  rename(arm_n = n) |> 
  arrange(age, design, year, study, arm_sort)
...
recurarization <- recur.dat %>%
  mutate(
    recur_np = n_percent_dig(recur_n, recur_n / arm_n * 100, 1),
    recur_p_bar = NA_character_,
    recur_p = recur_n / arm_n,
    arm = str_extract(arm, "\\w*"),
    recur_p_bar = ifelse(arm %in% c("Spont", "Placebo"), bar_wn1d(recur_p, recur_p, color_4, 1), recur_p_bar),
    recur_p_bar = ifelse(str_detect(arm, "Sug"), bar_wn1d(recur_p, recur_p, color_1, 1), recur_p_bar),
    recur_p_bar = ifelse(str_detect(arm, "Neo"), bar_wn1d(recur_p, recur_p, color_2, 1), recur_p_bar),
    recur_p_bar = str_replace(recur_p_bar, ">.*<", ">&nbsp;<"),
    arm_c = case_when(
      str_detect(arm, "Sug") ~ cell_spec(arm, color = color_1),
      str_detect(arm, "Plac") ~ cell_spec(arm, color = color_4),
      str_detect(arm, "Spont") ~ cell_spec(arm, color = color_4),
    ...
    # study = ifelse(!is.na(arm_sort), paste(study, arm_sort), study),
  ) %>%
  # filter(age == "Adult") %>%
  filter(!is.na(recur_np)) %>%
  arrange(age, design, year, study, arm_sort) |> 
  select(study, arm_n, arm_c, mg_kg, deep:min, recur_np, recur_p_bar)
...
recurarization %>% 
  ...
  mutate(study = ifelse(row_number() != 1, NA, study)) %>%
  kbl(
    booktabs = T, align = c("llllllllll"), escape = FALSE, # format = "latex",
    col.names = c("Study", "N", "Drug", "mg/kg", "Deep", "Moderate", "Shallow", "Minimal", "N (%)", bar_ref)
  ) %>%
  add_header_above(c(" " = 4, " Depth at Reversal" = 4, " " = 2), bold = TRUE, align = "c", line = TRUE) %>%
  row_spec(0, bold = TRUE, align = "l") %>%
  kable_styling(bootstrap_options = opt_boot, position = "left") %>%
  kable_classic(full_width = FALSE, html_font = opt_font, position = "left", "hover") %>%
  column_spec(1, width = 15) %>%
  column_spec(2, width = "4em") %>%
  column_spec(c(3, 4), width = "7em") %>%
  column_spec(5:8, width = "6em") %>%
  column_spec(9, width = "5em") %>%
  column_spec(10, width = bar_width) %>%
  pack_topnoind(., "Adult RCT", 1, 31) %>%
  pack_topnoind(., "Pediatric RCT", 32, 35) %>%
  footnote(
    # general = "NS: not specified; mod: moderate; endsurg: end of surgery; shall: shallow; min: minimal; oth: other ",
    # number = c(""),
    general_title = "",
...
<font size = 4> Figure `r figure_n`. Pooled risk difference for reparalysis randomized in RCTs comparing sugammadex with neostigmine. </font>

```{r recurarizationPooled, fig.width = 10, fig.height = 5}
## reintubMeta -------------------------------------- (2022-03-10 10:47) @----
# studies including neostigmine sugammadex comparison
# compare_refids <- recur.dat |> 
#   mutate(arm_sort = str_extract(arm, "\\(.*\\)"),
#          neo_sug = str_extract(arm, "\\w*")) |> 
#   arrange(study, arm_sort, arm) |> 
#   group_by(study, arm_sort) |> 
#   mutate(compare_arm = lead(neo_sug)) |> 
#   slice(1) |> 
#   ungroup() |> 
#   filter(neo_sug == "Neo" & compare_arm == "Sug") |> 
#   pull(refid)

recur_meta <- recur.dat %>%
filter(refid %in% compare_refids) |>
  filter(!(study == "He 2022" & mg_kg == "0.07")) |>
  mutate(
    # recur_n = ifelse(study == "He 2022" & arm == "Neo", 8, recur_n),
    # arm_n = ifelse(study == "He 2022" & arm == "Neo", 22, arm_n),
    recur_np = n_percent_dig(recur_n, recur_n / arm_n * 100, 1),
    recur_p_bar = NA_character_,
    recur_p = recur_n / arm_n,
    arm = str_extract(arm, "\\w*"),
  ...
  filter(!is.na(recur_np)) %>%
  arrange(age, design, year, study, arm_sort) |>
  select(study, arm_n, arm, recur_n) |>
  pivot_wider(id_cols = "study", names_from = "arm", values_from = c("recur_n", "arm_n")) |>
  mutate(
    arm_n_Neo = ifelse(study == "He 2022", 22, arm_n_Neo),
    recur_n_Neo = ifelse(study == "He 2022", 8, recur_n_Neo)
  )

# rct effect sizes
recur_rct_meta <- metabin(recur_n_Sug, arm_n_Sug, recur_n_Neo, arm_n_Neo,
  data = recur_meta,
  studlab = study,
  sm = "RD",
  pscale = 100,
...
study_char_table %>%
  ...
  pack_topnoind(., "RCT",                        1, 129) %>% 
  pack_topnoind(., "Before-After/Time Series", 130, 131) %>% 
  pack_topnoind(., "Nonrandomized Trial",      132, 142) %>% 
  pack_topnoind(., "Prospective Cohort",       143, 167) %>%
  pack_topnoind(., "Retrospective Cohort",     168, 183) %>% 
...
study_char_table %>%
  ...
  pack_topnoind(., "Retrospective Cohort", 5, 8) %>% 
  footnote(general_title = "",
...
study_char_table %>%
  ...
  column_spec(10, width = "5em") %>% 
  # column_spec(2, width = "2em") %>% 
  pack_topnoind(., "RCT", 1, 14) %>% 
  pack_topnoind(., "Prospective Cohort", 15, 18) %>% 
  pack_topnoind(., "Retrospective Cohort", 19, 19) %>% 
  footnote(general_title = "",
...
```{r ptCharTableAdult}
## ptCharTableData ------------------------------------ (2022-05-03 07:26) @----
# pt_char_table %>%
#   arrange(age, design, study) %>% 
#   filter(age == "Adult") %>% 
#   select(study, refid, age, design) %>% 
...
study_fund_table %>% 
  ...
  pack_topnoind(., "Prospective Cohort",       143, 167) %>%
  pack_topnoind(., "Retrospective Cohort",     168, 183) %>% 
  footnote(general = "Pub: public, Ind: industry; NR: not reported; MSD: Merck, Sharp, and Dohme (funded 27 (21%) of RCTs and 32 (17%) of studies listed).",
