...
```{r rnmbMetaRctByDepth, fig.width = 10, fig.height = 7.5}
# TODO: fig.height
...
<font size = 4> Figure `r figure_n`. Pooled residual neuromuscular block (TOFR <0.9) from randomized trials comparing sugammadex and neostigmine according to volatile anesthetic. </font>

```{r rnmbMetaRctByVolatile, fig.width = 10, fig.height = 6.5}
## rnmbMetaRctByVolatile -----------------------------+ (2022-04-03 10:09) @----
# NOTE: no HK adjustment owing to subgroup size
rnmb_meta <- metabin(lt_9_n_Sug, n_Sug, lt_9_n_Neo, n_Neo, 
  data = rnmb_meta.dat,
  studlab = study_orig,
  sm = "RR",
...
```{r metaTimeEndSurg, fig.width = 9, fig.height = 4.7}
forest_time(end_surg)
...
temp %>%
  ...
  pack_top(., "Moderate Block^a^", 11, 14, indent_tf = FALSE) %>% 
...
hypox_dat %>%
  ...
  pack_sub_bold(., "Retrospective Cohort", 40, 41) %>%
  footnote(
...
# funnel(hypox_meta)
```
No indication of potential small-study effects in funnel plot or regression tests.
...
<font size = 4> Table `r table_n`. Composite definitions of pulmonary complications. </font>
...
study_char_table %>%
  ...
  pack_topnoind(., "Quasi-Experimental", 121, 122) %>% 
  pack_topnoind(., "Nonrandomized Studies of Interventions", 123, 133) %>% 
...
pt_char_table %>% 
  ...
  pack_topnoind(., "RCT", 1, 120) %>% 
  pack_topnoind(., "Quasi-Experimental", 120, 122) %>% 
  pack_topnoind(., "Nonrandomized Studies of Interventions", 123, 133) %>% 
  pack_topnoind(., "Prospective Cohort", 134, 155) %>%
  pack_topnoind(., "Retrospective Cohort", 156, 164) %>% 
  footnote(general_title = "",
