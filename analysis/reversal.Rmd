...
figure_n <- fig_inc()
```
Risk difference -23.5% (95% CI, -36.2% to -10.8%). Too few studies to examine small study effects. <br/>
TOFR <0.7 (4 RCTs, n = 574) — RR 0.17 (95% CI, 0.08–0.39; I^2^ = 14%; 95% prediction interval, 0.02–1.48), RD -16.0% (95% CI, -23.8% to -8.2%).
...
minimal <- update.meta(temp, subset = depth_f == "Minimal", prediction = TRUE)
...
adverse.dat %>%
  ...
  pack_rows(., "Nonrandomized Trial", 45, 47, indent = FALSE) %>%
  pack_rows(., "Retrospective Cohort", 48, 48, indent = FALSE) %>%
  # pack_rows(., "Case Control", 9, 10) %>%
  footnote(
