...
included_studies.dat %>% 
  ...
  pack_top(., "Adult",                      1, 164) %>%
  pack_sub(., "RCT",                        1, 120) %>% 
  pack_sub(., "Before-After/Time Series", 121, 122) %>% 
  pack_sub(., "Nonrandomized Trial",      123, 133) %>% 
  pack_sub(., "Prospective Cohort",       134, 155) %>% 
  pack_sub(., "Retrospective Cohort",     156, 164) %>% 
  pack_top(., "All ages",                 165, 171) %>% 
  pack_sub(., "RCT",                      165, 168) %>% 
  pack_sub(., "Retrospective Cohort",     169, 171) %>% 
  pack_top(., "Pediatric",                172, 187) %>%
  pack_sub(., "RCT",                      172, 183) %>% 
  pack_sub(., "Prospective Cohort",       184, 186) %>% 
  pack_sub(., "Case-control",             187, 187) %>% 
  footnote(
...
figure_n <- fig_inc()
```
Risk difference -21.6% (95% CI, -33.8% to -9.4%); n.b., includes 11 studies (n = 1451), while RR estimate 8 studies (n = 1143). Too few studies to examine small study effects. Schaller 2010 included based on range of recovery times. 
...
time.dat <- suppressWarnings(read_csv("data/recoveryTime_050422.csv", col_types = c("cccccncccccccccccccncccccc"))) %>%
...
```{r timeDescrKblRct}
## timeDescrKblRct -----------------------------------+ (2022-05-05 09:53) @----
...
```{r timeResultsData}
## timeResultsData ------------------------------------ (2022-05-05 09:12) @----
...
## timeResultsRctAdult -------------------------------+ (2022-05-05 09:54) @----
...
temp %>%
  ...
  pack_top(., "Deep Block^a^", 1, 8, indent_tf = FALSE) %>% 
  pack_top(., "Moderate Block^b^", 9, 46, indent_tf = FALSE) %>% 
  pack_top(., "Shallow Block^a^", 47, 58, indent_tf = FALSE) %>% 
  pack_top(., "Minimal^a^", 59, 63, indent_tf = FALSE) %>% 
  pack_top(., "End of Surgery^a^", 64, 75, indent_tf = FALSE) %>% 
  pack_top(., "Other", 76, 77, indent_tf = FALSE) %>% 
  footnote(
...
## metaTime9 data ------------------------------------- (2022-05-05 09:56) @----
...
## arrhythmiaMeta ------------------------------------- (2022-05-03 07:22) @----
arrhyth <- adverse.dat %>% 
  filter(age %in% c("Adult", "All ages")) %>% 
  mutate(year = str_extract(study, "\\d{4}")) %>% 
  filter(!is.na(arrhyth_n)) %>%
...
## ponvMetaForest ------------------------------------- (2022-05-03 07:23) @----
# <!-- fig.asp = .5} -->
pairs.dat <- pairwise(
  treat = arm,
  event = ponv_n,
...
## nauseaMetaForest ----------------------------------- (2022-05-03 07:24) @----
pairs.dat <- pairwise(
  treat = arm,
  event = nausea_n,
  n = arm_n,
...
## vomitMetaForest ------------------------------------ (2022-05-03 07:25) @----
pairs.dat <- pairwise(
  treat = arm,
  event = vomit_n,
  n = arm_n,
...
hypox_dat %>%
  ...
  pack_topnoind(., "    SaO^2^ ≤90%", 1, 21) %>%
  pack_sub_bold(., "RCT", 1, 13) %>%
  pack_sub_bold(., "Interrupted Time Series", 14, 15) %>%
  pack_sub_bold(., "Nonrandomized Trials", 16, 17) %>%
  pack_sub_bold(., "Prospective Cohort", 18, 21) %>%
  pack_topnoind(., "    SaO^2^ 90-95%", 22, 39) %>%
  pack_sub_bold(., "RCT", 22, 39) %>%
  pack_topnoind(., "    SaO^2^ Not Specfified", 40, 41) %>%
  pack_sub_bold(., "RCT", 40, 41) %>%
  footnote(
...
<font size = 4> Table `r table_n`. Rates of recurarization. </font>

```{r recurar} 
## recurar -------------------------------------------- (2022-05-05 14:04) @----
recur.dat <- suppressMessages(read_csv("data/weakness.csv")) |> 
  clean_names() %>%
  remove_empty(which = "cols") %>%
  fill(refid:study) %>%
...
recurarization %>% 
  ...
  pack_topnoind(., "Adult RCT", 1, 45) %>%
  pack_topnoind(., "Pediatric RCT", 46, 49) %>%
  footnote(
    # general = "NS: not specified; mod: moderate; endsurg: end of surgery; shall: shallow; min: minimal; oth: other ",
    # number = c(""),
    general_title = "",
...
pt_char_table %>% 
  ...
  pack_topnoind(., "RCT",                        1, 120) %>%
  pack_topnoind(., "Before-After/Time Series", 121, 122) %>%
  pack_topnoind(., "Nonrandomized Trial",      123, 133) %>%
  pack_topnoind(., "Prospective Cohort",       134, 155) %>%
  pack_topnoind(., "Retrospective Cohort",     156, 164) %>%
  footnote(general_title = "",
