...
<a id="kq4Fig3"></a>Pooled RR 0.17 (95% CI: 0.06, 0.49; I^2^ = 66%)
...
```{r metaTimeShallow, fig.width = 10, fig.height = 4}
forest_time(shallow)
...
```{r metaTimeMinimal, fig.width = 10, fig.height = 3.3}
forest_time(minimal)
...
```{r metaTimeEndSurg, fig.width = 10, fig.height = 4.7}
forest_time(end_surg)
...
adverse.dat %>%
  ...
  pack_rows(., "RCT", 1, 40, indent = FALSE) %>%
  pack_rows(., "Nonrandomized Trial", 41, 42, indent = FALSE) %>%
...
```{r pooledBrady, fig.width = 10, fig.height = 5}
...
ponv.dat %>%
  ...
  mutate(
    ...
    ponv_p_bar = ifelse(arm %in% c("Spont", "Placebo"), bar_wn1d(ponv_p, ponv_p, color_4, 100), ponv_p_bar),
  ...
  filter(!is.na(ponv_n)) %>%
  arrange(design, year, refid) %>%
  select(study, arm_n, arm_c, mg_kg, ponv_np, ponv_p_bar) %>%
  group_by(study) %>%
  mutate(study = ifelse(row_number() != 1, NA, study)) %>%
  kbl(
    booktabs = T, align = c("llllll"), escape = FALSE, # format = "latex",
    col.names = c("Study", "N", "Agent", "mg/kg <u>mg</u>", "PONV, N (%)", " ")
  ) %>%
  row_spec(0, bold = TRUE, align = "l") %>%
  kable_styling(bootstrap_options = opt_boot, position = "left") %>%
  kable_classic(full_width = FALSE, html_font = opt_font, position = "left", "hover") %>%
  ...
  column_spec(5, width = "11em") %>%
  column_spec(6, width = "10em") %>%
  pack_rows(., "RCT", 1, 30, indent = FALSE) %>%
  pack_rows(., "Nonrandomized Trial", 31, 32, indent = FALSE) %>%
  pack_rows(., "Retrospective Cohort", 33, 34, indent = FALSE) %>%
  # pack_rows(., "Case Control", 9, 10) %>%
  footnote(
    # general = "IBW: ideal body weight.",
    #        number = c("● Limited, ●● Important, ●●● Critical."),
...
nma_ponv <- ponv.dat %>%
  ...
  mutate(
    ...
    arm = ifelse(arm %in% c("Spont", "Placebo"), "Spont/Plac", arm),
...
```{r ponvNodeSplit, include = TRUE, echo = FALSE, fig.align = 'left', fig.width = 8, fig.height = 5}
...
ponv.dat %>%
  ...
  mutate(
    ...
    nausea_p_bar = ifelse(arm %in% c("Spont", "Placebo"), bar_wn1d(nausea_p, nausea_p, color_4, 100), nausea_p_bar),
  ...
  kbl(
    booktabs = T, align = c("llllll"), escape = FALSE, # format = "latex",
    col.names = c("Study", "N", "Agent", "mg/kg <u>mg</u>", "Nausea, N (%)", " ")
  ) %>%
  row_spec(0, bold = TRUE, align = "l") %>%
  kable_styling(bootstrap_options = opt_boot, position = "left") %>%
  kable_classic(full_width = FALSE, html_font = opt_font, position = "left", "hover") %>%
...
```{r nauseaNodeSplit, include = TRUE, echo = FALSE, fig.align = 'left', fig.width = 8, fig.height = 5}
...
ponv.dat %>%
  ...
  mutate(
    ...
    vomit_p_bar = ifelse(arm %in% c("Spont", "Placebo"), bar_wn1d(vomit_p, vomit_p, color_4, 100), vomit_p_bar),
  ...
  kbl(
    booktabs = T, align = c("llllll"), escape = FALSE, # format = "latex",
    col.names = c("Study", "N", "Agent", "mg/kg <u>mg</u>", "Vomiting, N (%)", " ")
  ) %>%
  row_spec(0, bold = TRUE, align = "l") %>%
  kable_styling(bootstrap_options = opt_boot, position = "left") %>%
  kable_classic(full_width = FALSE, html_font = opt_font, position = "left", "hover") %>%
...
```{r vomitNodeSplit, include = TRUE, echo = FALSE, fig.align = 'left', fig.width = 8, fig.height = 5}
...
<font size = 4> Table `r table_n`. Composite pulmonary complications. </font>
...
composite <- popc.dat %>%
  ...
  mutate(
    ...
    composite_p_bar = ifelse(arm %in% c("Spont", "Placebo"), bar_wn1d(composite_p, composite_p, color_4, 1), composite_p_bar),
  ...
  arrange(design, year, refid) %>%
  select(study, arm_n, arm_c, mg_kg, depth_rev_f, composite_np, composite_p_bar, design, arm)
...
composite %>% 
  ...
  kbl(
    booktabs = T, align = c("lllllll"), escape = FALSE, # format = "latex",
    col.names = c("Study", "N", "Agent", "mg/kg", "Reversal", "Composite, N (%)", " ")
  ) %>%
  add_header_above(c(" " = 4, " Depth at" = 1, " " = 2), bold = TRUE, align = "l", line = FALSE) %>%
  row_spec(0, bold = TRUE, align = "l") %>%
  kable_styling(bootstrap_options = opt_boot, position = "left") %>%
  ...
  column_spec(6, width = "11em") %>%
  column_spec(7, width = "10em") %>%
  pack_topnoind(., "RCT", 1, 23) %>%
  pack_topnoind(., "Nonrandomized Trials", 24, 25) %>%
  pack_topnoind(., "Prospective Cohort", 26, 34) %>%
  pack_topnoind(., "Retrospective Cohort", 35, 38) %>%
  # result_pack(., "")
  footnote(
    general = "NS: not specified; mod: moderate; endsurg: end of surgery; shall: shallow; min: minimal; oth: other ",
    # number = c(""),
...
<font size = 4> Figure `r figure_n`. Pooled incidence of postoperative pulmonary complications in randomized and nonrandomized studies, and component endpoints included in each study's defintion. </font>

```{r compositeMeta, fig.width = 12, fig.height = 5.6}
## compositeMeta -------------------------------------- (2022-03-10 07:43) @----
# add composite definition
comp_defn <- readxl::read_xlsx("data/POPCRevAgent_030922.xlsx", sheet = "comp_defn", range = "A1:C11") %>% 
...
grid::grid.text("Sugammadex  Neostigmine", .285, .955, gp = grid::gpar(cex = 1.1))

figure_n <- tab_inc()
```
Results from 3 nonrandomized studies adjusted (Li 2021 using propensity scores, Kirmeir 2018 multivariable regression with a likely sufficient set of deconfounders, and Kheterpal 2020 in a matched cohort design).
...
popc.dat %>%
  ...
  mutate(
    ...
    pneumonia_p_bar = ifelse(arm %in% c("Spont", "Placebo"), bar_wn1d(pneumonia_p, pneumonia_p, color_4, 1), pneumonia_p_bar),
  ...
  arrange(design, year, refid) %>%
  select(study, arm_n, arm_c, mg_kg, depth_rev_f, pneumonia_np, pneumonia_p_bar) %>%
  group_by(study) %>%
  mutate(study = ifelse(row_number() != 1, NA, study)) %>%
  kbl(
    booktabs = T, align = c("lllllll"), escape = FALSE, # format = "latex",
    col.names = c("Study", "N", "Agent", "mg/kg", "Reversal", "Pneumonia, N (%)", " ")
  ) %>%
  add_header_above(c(" " = 4, " Depth at" = 1, " " = 2), bold = TRUE, align = "l", line = FALSE) %>%
  row_spec(0, bold = TRUE, align = "l") %>%
  kable_styling(bootstrap_options = opt_boot, position = "left") %>%
  ...
  pack_topnoind(., "Retrospective Cohort", 16, 23) %>%
  pack_topnoind(., "Case-Control", 24, 25) %>%
  footnote(
    general = "NS: not specified; mod: moderate; endsurg: end of surgery; shall: shallow; min: minimal; oth: other ",
    # number = c(""),
    general_title = "",
...
popc.dat %>%
  ...
  mutate(
    ...
    resp_fail_p_bar = ifelse(arm %in% c("Spont", "Placebo"), bar_wn1d(resp_fail_p, resp_fail_p, color_4, 1), resp_fail_p_bar),
