...
<font size = 4> Table `r table_n`. Results from studies reporting residual neuromuscular blockade. Bars proportional to percentages (<b><font color = "#f1948a">clinical alone</font>, <font color = "#f5c800">any PNS</font>, or <font color = "#58d68d">quantitative</font></b> monitoring). </font> <font size = 3> (Limited importance time-related outcomes—time from extubation to TOFR 0.9—reported in only 2 studies and not summarize). </font>
...
nmm_kable <- nmm_kable %>% 
  ...
  tofr_bar = case_when(
    ...
    arm_diag_outcome %in% c("AMG (nl)", "AMG (raw)", "AMG", "EMG") ~ color_bar("#58d68d", fun = dichot_format)(rnmb1_per),
...
contin_ptout.dat %>%
  ...
  pack_top(., "RCT", 1, 17) %>%
  pack_top(., "Quasi-Experimental", 18, 19) %>% 
  footnote(
...
<font size = 4> Table `r table_n`. Results of studies reporting pulmonary complications. Bars proportional to percentages (<b><font color = "#f1948a">clinical alone</font>, <font color = "#f5c800">any PNS</font>, <font color = "#aec176">any monitoring</font>, or <font color = "#58d68d">quantitative</font></b> monitoring). </font>
...
dichot_pulm_kbl %>% 
  ...
  pack_sub_bold(., "Quasi-Experimental", 3, 4) %>%
  pack_sub_bold(., "Prospective Cohort", 5, 6) %>%
  # upper airway obstruction
  pack_top(., "Upper Airway Obstruction", 7, 8) %>%
  pack_sub_bold(., "RCT", 7, 8) %>%
  # resp failure
  pack_top(., "Respiratory Failure", 9, 10) %>%
  pack_sub_bold(., "Quasi-Experimental", 9, 10) %>%
  # hypoxia
  pack_top(., "Hypoxia", 11, 16) %>%
  pack_sub_bold(., "RCT", 11, 16) %>%
  # bronchospasm
  pack_top(., "Brochospasm", 17, 18) %>%
  pack_sub_bold(., "RCT", 17, 18) %>%
  # pnuemonia
  pack_top(., "Pneumonia", 19, 20) %>%
  pack_sub_bold(., "Prospective Cohort", 19, 20) %>%
  # pulmonary
  pack_top(., "Pulmonary", 21, 24) %>%
  pack_sub_bold(., "Prospective Cohort", 21, 24) %>%
  footnote(
