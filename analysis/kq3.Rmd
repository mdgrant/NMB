...
bar_ref <- "<span style='display: inline-block; unicode-bidi: plaintext; background: linear-gradient(270deg, transparent 43.00% , #e0e0de 43.00% , #e0e0de 100.00% , transparent 100.00% ); width: 163%'>&nbsp;</span>"
# <span style="display: inline-block; unicode-bidi: plaintext; background: linear-gradient(270deg, transparent  43.00% , #f1948a 43.00% , #e0e0de 100.00% , transparent 100.00% ); width: 100%",>&nbsp;</span>
...
# write_csv(as_tibble(ptout_refids), "data/ptout_refids.csv", na = "")
...
included_studies.dat %>% 
  ...
  # pack_top(., "Adult", 1, 14) %>%
  pack_top(., "RCT", 1, 10) %>% 
  pack_top(., "Quasi-Experimental", 11, 11) %>% 
  pack_top(., "Prospective Cohort", 12, 13) %>% 
  pack_top(., "Retrospective Cohort", 14, 14) %>% 
  footnote(
    general = "RCT: randomized controlled trial.",
    alphabet = c(
...
nmm_kable <- dichot_ptout_tofr.dat %>%
  ...
  ungroup() %>%
    ...
    rnmb_location = case_when(
      rnmb_location == "or" ~ "<i>OR</i>",
      rnmb_location == "post_extub" ~ "<i>OR</i>",
      rnmb_location == "pacu" ~ "<i>PACU</i>",
      rnmb_location == "recover" ~ "<i>PACU</i>",
    ),
...
#   select(study, arm_diag_outcome, arm_n, rnmb_np, tofr_bar, tofr_outcome, rnmb_location, or_ci) %>%
nmm_kable %>%
  select(study_l, arm_diag_outcome, arm_n, rnmb_np, tofr_bar, tofr_outcome, rnmb_location, or_ci) %>%
  ...
  kbl(
    ...
    col.names = c("Study", "Monitoring", "N", "N (%)", bar_ref, "TOFR", "Place^a^", "OR (95% CI)^b^")
  ) %>%
  ...
  pack_topnoind(., "Quasi-Experimental", 18, 19) %>%
  # pack_top(., "Prospective Cohort", 13, 13) %>%
  pack_topnoind(., "Retrospective Cohort", 20, 21) %>%
  footnote(
    general = "RNMB: residual neuromuscular block; OR: odds ratio; CI: confidence interval; RCT: randomized controlled trial; TOFR: train of four ratio; DBS: double burst stimulation; PNS: peripheral nerve stimulator; AMG: acceleromyography; EMG: electromyography; *OR*: operating room; nl: normalized.",
    alphabet = c(
      "PACU if desginated as PACU or recovery, *OR* if designated as such or as post-extubation.",
      "Odds ratio comparing the last monitoring arm (lowest incidence) to the prior. For studies reporting results for >1 threshold, only those for the highest are shown."
    ),
    general_title = "",
    footnote_as_chunk = FALSE
  )

# as_image(document_kable, file = "/Users/mgrant/Documents/_projects01/asa/_nmb/figures/nmm_outcome1.png")
...
```{r forSOE, echo = FALSE}
## forSOE (updated 2022/01/21 19:13) --------------------------------------
# data
library(netmeta)
...
```{r quanttClinical, fig.width = 10, fig.height = 3}
temp.meta.dat <- pairs_temp.dat %>% 
...
forest(temp.meta,
  ...
  xlim = c(0.01, 5),
  at = c(.01, .1, .5, 1.0, 1.5),
  xlab = "Favors: Quantitative                Clinical")
...
<font size = 4> Figure `r figure_n`. Network plot of 10 studies (8 RCTs, 2 non-randomized) eporting residual neuromuscular block (TOFR <0.9 to <0.7). </font>
...
<font size = 4> Figure `r figure_n`. Pooled residual neuromuscular block (TOFR <0.9 to <0.7) from 8 randomized trials, 1 quasi-experimental, and 1 non-randomized study comparing quantitative, qualitative, and clinical monitoring (network meta-analysis). </font>
...
<font size = 4> Table `r table_n`. League table for relative risks comparing upper left to lower right. Network comparisons in lower left triangle and direct comparisons in upper right. </font>
...
```{r nmmNodeSplit, include = TRUE, echo = FALSE, fig.align = 'left', fig.width = 8, fig.height = 5}
netsplit(nmm_netmeta_nma) %>% forest()
...
dichot_pulm_kbl %>% 
  ...
  pack_topnoind(., "Upper Airway Obstruction", 7, 8) %>%
  pack_sub_bold(., "RCT", 7, 8) %>%
  # hypoxia
  pack_topnoind(., "Hypoxia", 7, 12) %>%
  ...
  pack_sub_bold(., "Prospective Cohort", 17, 20) %>%
  footnote(
...
study_char_table %>%
  ...
  pack_topnoind(., "Quasi-Experimental", 11, 11) %>% 
  pack_topnoind(., "Prospective Cohort", 12, 13) %>%
  pack_topnoind(., "Retrospective Cohort", 14, 15) %>% 
