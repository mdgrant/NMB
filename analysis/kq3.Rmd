...
included_studies.dat %>% 
  ...
  # pack_top(., "Adult", 1, 14) %>%
  pack_top(., "RCT", 1, 10) %>% 
  pack_top(., "Before-After", 11, 11) %>% 
  pack_top(., "Prospective Cohort", 12, 14) %>% 
...
## *Residual Neuromuscular Blockade (TOFR)*
...
nmm_kable %>%
  ...
  footnote(
    general = "RCT: randomized controlled trial; RNMB: residual neuromuscular blockade; OR: odds ratio; CI: confidence interval; RCT: randomized controlled trial; TOFR: train of four ratio; DBS: double burst stimulation; PNS: peripheral nerve stimulator; AMG: acceleromyography; EMG: electromyography; OR: operating room.",
    alphabet = c(
...
study_char_table %>%
  ...
  pack_topnoind(., "Prospective Cohort", 14, 14) %>% 
  pack_topnoind(., "Retrospective Cohort", 15, 15) %>% 
